...
function configuredTextPrice(text, source) {
  if (!source.priceRegex) return null;
  try {
...
function productSnapshot(html, source) {
  ...
  const textPrice = configuredTextPrice(text, source);

  let rawPrice = source.preferPriceRegex && textPrice !== null
    ? textPrice
    : numberFromText(offer?.price ?? offer?.lowPrice ?? offer?.priceSpecification?.price);
  ...
  if (rawPrice === null && textPrice !== null) rawPrice = textPrice;
  if (rawPrice === null) {
    return {
      ok: false,
...
function shippingSnapshot(html, source) {
  ...
  let match;
  try {
    match = text.match(new RegExp(source.priceRegex, "i"));
  } catch {
    return { ok: false, monitorStatus: "parser_config_error", identityStatus: "verified", error: "Shipping priceRegex is invalid." };
  }
