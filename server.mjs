...
http.createServer(async(req,res)=>{try{const url=new URL(req.url,`http://${req.headers.host||"localhost"}`);
  if(url.pathname==="/health")return send(res,200,JSON.stringify({ok:true,service:"dropi-global-deals",catalogVersion:3}));
  if(url.pathname==="/api/products"&&req.method==="GET")return send(res,200,JSON.stringify(enrichCatalog(await readJson("data/products.json"))));
